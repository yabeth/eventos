...
use App\Exports\CertificadosPorEventoExport;
...
class ReportesController extends Controller
{
    ...
    public function exportCertificadosExcel(Request $request)
    {
        $idevento = $request->ideven;

        if (!$idevento) {
            return redirect()->back()->with('error', 'Seleccione un evento válido.');
        }

        $nombreEvento = \App\Models\Evento::findOrFail($idevento)->eventnom;

        $filename = 'Certificados_' . str_replace([' ', '/'], '_', $nombreEvento) . '_' . date('d-m-Y') . '.xlsx';

        return Excel::download(new CertificadosPorEventoExport($idevento), $filename);
    }
}
