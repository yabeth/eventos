...
<br><br>
    <blockquote>
        <h4 style="position: relative; top: -30px; line-height: 1.6; font-size: 16px">
            Listado de Usuarios de SISCERTI
        </h4>
    </blockquote>
    ...
    <div class="table-container" style="position: relative; top: -30px;">
        <table class="table table-bordered table-sm table-font-size">
            ...
            <tbody>
                @foreach($usuarios as $index => $permi)
                <tr>
                    <td style="text-align:center">{{ $index + 1 }}</td>
                    <td>{{ $permi->nomusu }}</td>
                    <td>{{ $permi->tipousuario->tipousu }}</td>
                    <td> @if($permi->permisos->isEmpty())
        <p><em>Sin permisos</em></p>
    @else
        <ul>
            @foreach($permi->permisos as $permiso)
                <li>{{ $permiso->nombre_permiso }}</li> <!-- Asume que `nombre_permiso` es el nombre del permiso en el modelo `Permiso` -->
            @endforeach
        </ul>
    @endif</td>
