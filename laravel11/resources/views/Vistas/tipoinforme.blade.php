...
<div id="tabla-tipeven">
    <div class="container mt-2">
        <div class="card">
            <div class="card-header">
                <h5 class="text-center">Tipo de informes</h5>
            </div>
            <div class="container">
                                    ...
                                    <td>
                                        <center>
                                            <div class="action-buttons">
                                                <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#edit{{$tipoinforme->idTipinfor}}">
                                                    <i class="bi bi-pencil"></i>
                                                </button>
                                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#delete{{ $tipoinforme->idTipinfor }}">
    ...
    <div class="modal-dialog">
        ...
        <form action="{{ route('Rut.tipinfo.store') }}" method="post">
                    @csrf
                    <div class="modal-header bg-primary text-white">
                        <h5 class="modal-title" id="addEmployeeModlLabel">Agregar nuevo Tipo de informe</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <label for="nomtinform" class="form-label">Tipo de informe</label>
                            <input type="text" class="form-control" name="nomtinform" id="nomtinform">
                        </div>
                    </div>
    ...
    <div id="edit{{ $tipoinforme->idTipinfor }}" class="modal fade" tabindex="-1" aria-labelledby="editModalLabel">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('Rut.tipinfo.update', $tipoinforme->idTipinfor) }}" method="POST" >
                    @csrf
                    @method('PUT')
                    
                    <div class="modal-header bg-primary text-white">            
                    <h4 class="modal-title" id="editModalLabel">Editar tipo de informe</h4>  
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="nomtinform" class="form-label">Tipo de informe</label>
                            <input type="text" class="form-control" name="nomtinform" id="nomtinform" value="{{ $tipoinforme->nomtinform }}">
                        </div>
                    </div>
                    <div class="modal-footer">
    ...
    <!-- Modal Eliminar -->
    @foreach($tipoinformes as $tipoinforme)
<div id="delete{{ $tipoinforme->idTipinfor }}" class="modal fade" tabindex="-1" aria-labelledby="deleteModalLabel">
    <div class="modal-dialog modal-sm">
        <div class="modal-content text-center">
            <form action="{{ route('Rut.tipinfo.destroy', $tipoinforme->idTipinfor) }}" method="POST">
            @csrf
            @method('DELETE')
                ...
                <div class="modal-footer border-0 justify-content-center pt-0 pb-3">
                    <button type="button" style="cursor:pointer;" class="btn btn-warning" data-bs-dismiss="modal" aria-hidden="true">Cancelar</button>
...
document.getElementById('btnCancelar').addEventListener('click', function() {
        document.getElementById('nomtinform').value = ''; 
    });
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: data.message || 'Ocurrió un error al intentar guardar el tipo de informr',
                });
            }
        })
        .catch(error => {
