...
<head>
    ...
    <title>Eventos por rango de fecha</title>
    @php
    use Carbon\Carbon;
    @endphp
    ...
    <hr style="height: 0.5px;border: 0;background-color: black;">       
        <blockquote>
            <h3 style="position: relative; top: -35px; line-height:1.6;">
                Listado de eventos por fechas:
            </h3>
        </blockquote>

        <blockquote>
            <h4 style="position: relative; top: -35px; line-height:0.4;">
                Desde  {{$fecinicio}}  hasta  {{$fecfin}}:
            </h4>
        </blockquote>
        ...
        <table class="table table-bordered table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $evento->estadoEvento->nomestado }}</td>
                    <td style="width: 75px;">{{ Carbon::parse($evento->fecini)->format('d-m-Y') }}</td>
                    <td>{{ $evento->horain }}</td>
                </tr>
                @endforeach
            </tbody>
