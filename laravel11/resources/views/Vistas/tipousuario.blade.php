    ...
    <div class="container-fluid mt-3">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered table-hover" id="my-table" cellspacing="0" width="100%">
                ...
                <tbody>
                    @foreach ($tipousuarios as $index =>  $tip) 
                    <tr>
                        <td>{{ $index + 1 }}</td> 
                        <td>{{ $tip->tipousu}}</td>
                        <td>
                            <div class="action-buttons">
                                <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#edit{{$tip->idTipUsua}}"><i class="bi bi-pencil"></i></button>
                                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"  data-target="#delete{{$tip->idTipUsua}}"><i class="bi bi-trash"></i></button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
...
<div id="addEmployeeModl" class="modal fade"  tabindex="-1" aria-labelledby="addEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('Rut.tipusu.store') }}" method="post">
                @csrf
                <div class="modal-header bg-primary text-white">
                    <h4 class="modal-title" id="addEmployeeModalLabel">Agregar nuevo tipo de usuario</h4>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="tipousu" id="tipousu" required>
                    </div>
                </div>
...
<div id="edit{{ $tip->idTipUsua }}" class="modal fade" tabindex="-1" aria-labelledby="editModalLabel">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form action="{{ route('Rut.tipusu.update', $tip->idTipUsua) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="editModalLabel">Editar tipo de usuario</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="color: white;">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="tipousu" class="form-label">Tipo de usuario</label>
                        <input type="text" class="form-control" name="tipousu" id="tipousu" value="{{ $tip->tipousu}}" required>
                    </div>
                </div>
                <div class="modal-footer">
...
@foreach ($tipousuarios as $tip) 
<div id="delete{{ $tip->idTipUsua}}" class="modal fade" tabindex="-1" aria-labelledby="deleteModalLabel">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('Rut.tipusu.destroy', $tip->idTipUsua) }}" method="POST">
                @csrf
                @method('DELETE')
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="deleteModalLabel">Eliminar tipo de usuario</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="color: white;">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    Estás seguro de eliminar a <strong>{{ $tip->tipousu}}</strong>?
                </div>
...
document.getElementById('btnCancelar').addEventListener('click', function() {
        document.getElementById('tipousu').value = ''; 
    });
